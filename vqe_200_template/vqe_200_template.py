...
def variational_ansatz(params, wires):
    ...
    # QHACK #
    # general W state preparation
    n_qubits = len(wires)
    qml.Rot(0, params[0]/n_qubits, np.pi, wires=0)
    for i in range(n_qubits-2):
        qml.CRot(0, params[i+1]/(n_qubits-i-1), np.pi,wires=[i,i+1])
    for i in range(n_qubits-1):
        qml.CNOT(wires=[n_qubits-(i+2),n_qubits-(i+1)])
    qml.RX(np.pi,wires=0)
    for i in range(n_qubits//2 - 1):
        qml.RZ(np.pi/2,wires=2*i +1)
    if n_qubits%2 ==1:
...
def run_vqe(H):
    ...
    params = np.random.normal(loc=np.pi/2, scale=0.1, size=(num_param_sets))
