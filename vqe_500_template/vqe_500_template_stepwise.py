...
def find_excited_states(H):
    ...
    def cost_fn01(params, **kwargs): 
        return cost_fn0(params, **kwargs)+ 0.75* cost_fn1(params, **kwargs)
    def cost_fn012(params, **kwargs): 
        return cost_fn0(params, **kwargs)+ cost_fn1(params, **kwargs)+ 0.75*cost_fn2(params, **kwargs)
    ...
    for n in range(max_iterations):
        params, prev_w_energy = opt.step_and_cost(cost_fn0, params)
        w_energy = cost_fn0(params)
        ...
        if conv <= conv_tol:
            break
    print (time.time() - t)
    opt = qml.NesterovMomentumOptimizer(stepsize=0.02)
    for n in range(max_iterations):
        params, prev_w_energy = opt.step_and_cost(cost_fn01, params)
        w_energy = cost_fn01(params)
        conv = np.abs(w_energy - prev_w_energy)
        if n % 20 == 0:
            print('Iteration = {:},  Energy = {:.8f} Ha'.format(n, w_energy))
        if conv <= conv_tol:
    ...
    for n in range(max_iterations):
        params, prev_w_energy = opt.step_and_cost(cost_fn012, params)
        w_energy = cost_fn012(params)
        conv = np.abs(w_energy - prev_w_energy)
        if n % 20 == 0:
            print('Iteration = {:},  Energy = {:.8f} Ha'.format(n, w_energy))
        if conv <= conv_tol:
