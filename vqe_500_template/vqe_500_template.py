...
def find_excited_states(H):
    ...
    def ture_cost_fn(params, **kwargs): 
        return 3*cost_fn0(params, **kwargs)+ 2*cost_fn1(params, **kwargs)+ cost_fn2(params, **kwargs)
    
    mt0 = qml.metric_tensor(cost_fn0) 
    mt1 = qml.metric_tensor(cost_fn1) 
    mt2 = qml.metric_tensor(cost_fn2)
    def mt(params, **kwargs): 
        return 3*mt0(params, **kwargs)+ 2*mt1(params, **kwargs)+ mt2(params, **kwargs)
    
    opt = qml.QNGOptimizer(stepsize=0.06)
    max_iterations = 500
    
    t = time.time()

    params = np.random.uniform(low=-np.pi / 2, high=np.pi / 2, size=(num_param_sets, 3))
    prev_w_energy = ture_cost_fn(params)
    for n in range(max_iterations+1):
        params = opt.step(ture_cost_fn, params,metric_tensor_fn=mt)
        w_energy = ture_cost_fn(params)
        conv = np.abs(w_energy - prev_w_energy)
        prev_w_energy = w_energy
        if n % 20 == 0:
