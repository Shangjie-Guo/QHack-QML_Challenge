...
def find_excited_states(H):
    ...
    def variational_ansatz(params, wires):
        qml.ArbitraryStatePreparation(params, wires)
    ...
    num_param_sets = (2 ** (num_qubits+1)) - 2
    ...
    params = np.random.uniform(low=-np.pi, high=np.pi, size=(num_param_sets, 3))
    i_energy = np.sum(cost_fn0(params)+cost_fn1(params)+cost_fn2(params))
    for i in range(10):
        p = np.random.uniform(low=-np.pi, high=np.pi, size=(num_param_sets, 3))
        cost_fn = set_w_cost_fn((i+1)*0.08)
        for j in range(28):
            p = opt.step(cost_fn, p)
        e = np.sum(cost_fn0(p)+cost_fn1(p)+cost_fn2(p))
        if e < i_energy:
            cost_fn_f = cost_fn
            params = p
            i_energy = e
            
    opt1 = qml.AdamOptimizer(stepsize=0.2)
    for n in range(max_iterations+1):
        params = opt1.step(cost_fn_f, params)
