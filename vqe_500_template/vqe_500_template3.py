...
def find_excited_states(H):
    ...
    def ture_cost_fn(params, **kwargs): 
        p = np.reshape(params,(num_param_sets, 3))
        return 3*cost_fn0(p, **kwargs)+ 2*cost_fn1(p, **kwargs)+ cost_fn2(p, **kwargs)
    ...
    params.reshape(3*num_param_sets)
    param_opt = np.reshape(minimize(ture_cost_fn, params,method='BFGS')['x'],(num_param_sets, 3))
    
    energies[2] = cost_fn2(param_opt)
    energies[1] = cost_fn1(param_opt)
    energies[0] = cost_fn0(param_opt)
    energies.sort()
    print (time.time() - t)
    # QHACK #
