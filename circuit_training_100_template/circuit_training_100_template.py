...
def optimize_circuit(params):
    ...
    def cost_fn(x): # Wrap QNode object back into a cost function
        return circuit(x)
    
    opt = qml.GradientDescentOptimizer(stepsize=0.4)
    p = params # intitial guess
    
    steps = 100
    for i in range(steps):
        p = opt.step(cost_fn, p)
    optimal_value = cost_fn(p)
    ...
    #     p_new = opt.step(cost_fn, p)
    #     diff = np.abs(cost_fn(p_new) - cost_fn(p))
    #     p = p_new
